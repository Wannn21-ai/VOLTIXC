...
if (document.body.dataset.skipDevicePlaceholder !== "true") {
  try {
    ...
    if (!currentDevice) {
      document.getElementById("placeholder-title").textContent = tr("noDevicePairedYet");
      document.getElementById("placeholder-sub").textContent =
        tr("pairDeviceToMonitor");
    }
  } catch (error) {
    document.getElementById("placeholder-title").textContent = tr("deviceAccessUnavailable");
